...
const _processPage = (async (convertAction, urlOrHtml, options) => {
  ...
  try {
    ...
    if (urlOrHtml.match(/^http/i)) {
      ...
      await page.setContent(urlOrHtml, { waitUntil: "domcontentloaded" })
    }
...
function _handleError(error) {
  if (error instanceof Error) {
    process.stdout.write(
      JSON.stringify(['err_handleErrorX', error, error.name, error.message, error.stack, error.fileName, error.cause])
    );
  } else {
    process.stdout.write(JSON.stringify(['err_handleErrorY', error]));
  }
  process.stdout.write("\n");
}
